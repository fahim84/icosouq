                ...
                <?php foreach($rows->result() as $row)
                {
                    ...
                    <tr>
                        <td>
                            ...
                            <div style="width: 300px;"><?php $tags = explode(',',$row->tags); foreach ($tags as $tag) echo '<span class="label label-inverse">'.$tag.'</span>'; ?></div>
                            <div ><?php echo $row->about; ?></div>
                        </td>
                        <td>
                            <a href="<?php echo base_url(); ?>admin/template/update/?id=<?php echo $row->template_id; ?>" class="btn btn-transparent btn-xs"><i class="fa fa-pencil"></i> Edit</a>
                            <!--<a href="#_" id="<?php /*echo $row->template_id; */?>" class="btn btn-transparent btn-xs tooltips delete_button" data-toggle="modal" data-target="#DeleteModal" ><i class="fa fa-times fa fa-white"></i> Delete</a>-->
                            <div class="checkbox">
                                <input id="<?php echo $row->template_id; ?>" value="<?php echo $row->template_id; ?>" type="checkbox" class="js-switch" <?php echo $row->status ? 'checked' : ''; ?> />
                            </div>
                        </td>
                    </tr>
                <?php } ?>
...
<div class="modal fade" id="DeleteModal">
  <div class="modal-dialog">
    ...
    <form action="<?php echo base_url(); ?>admin/template/delete" method="get" name="delete_model_form" class="form-horizontal" id="delete_model_form" role="form">
...
$(document).on('change','.js-switch',function (e) {
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo base_url(); ?>admin/template/change_status',
